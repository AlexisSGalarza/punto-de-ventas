...
productos = [
    ...
    ( "Arroz", "Paquete de 1kg", "PRD001", "25.00", "Granos", "Proveedor A", "PV001"),
    ( "Azúcar", "Paquete de 1kg", "PRD002", "20.00", "Endulzantes", "Proveedor B", "PV002"),
    ( "Leche", "Caja de 1L", "PRD003", "22.00", "Lácteos", "Proveedor C", "PV003"),
    ( "Pan", "Bolsa de 10 piezas", "PRD004", "18.00", "Panadería", "Proveedor D", "PV004"),
    ( "Aceite", "Botella de 1L", "PRD005", "45.00", "Aceites", "Proveedor E", "PV005"),
    # Generar dinámicamente productos con sus IDs únicos
    *[
        (f"ID-{str(i).zfill(3)}", f"Producto {i}", f"Descripción {i}", f"PRD{str(i).zfill(3)}", f"{10 + i}.00", "Categoría", f"Proveedor {i}", f"PV{str(i).zfill(3)}")
        for i in range(6, 21)
    ]
]
...
class TiendaApp(ctk.CTk):
    def __init__(self):
        ...
        self.add_product_button = ctk.CTkButton(self.search_frame, text="➕ Agregar Producto", fg_color="#2ecc71", text_color="white", command=self.agregar_producto)
    ...
    def populate_table(self):
        ...
        # Encabezados de la tabla, incluyendo el ID del Producto
        headers = ["ID Producto", "Producto", "Descripción", "Código Producto", "Precio ($)", "Categoría", "Proveedor", "Código Proveedor", "Acciones"]
        ...
        for col in range(len(headers)):
            if col == len(headers)-1:  # Última columna (botones)
                ...
                self.frame_table.grid_columnconfigure(col, weight=2 if col < 2 else 1)
        ...
        for row, item in enumerate(page_items, start=1):
            # Generar un ID único para cada producto en la primera columna
            id_producto = f"ID-{start + row}"

            # Insertar el ID en la primera columna
            lbl_id = ctk.CTkLabel(self.frame_table, text=id_producto, font=("Arial", 14), text_color="black", padx=2)
            ...
            for col, value in enumerate(item):
                ...
                lbl.grid(row=row, column=col + 1, sticky="nsew", pady=2)  # El "+1" ajusta porque la primera columna es el ID
    ...
    def agregar_producto(self):
        """Función de agregar producto aún por implementar."""
        print("Función de agregar producto aún por implementar...")
